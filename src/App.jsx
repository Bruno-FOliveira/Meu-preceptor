...
function SetupScreen({onDone}){
  ...
  const test=async()=>{
    ...
    try{
      workingModel=null;localStorage.removeItem("geminiModel");localStorage.setItem("geminiKey",key.trim());
...
function NewPatientForm({onSave,onCancel}){
  const [f,setF]=useState({name:"",age:"",gender:"M",bed:"",location:"",status:"stable",summary:"",admissionDate:new Date().toISOString().split("T")[0],onAntibiotic:false,covering:false,pending:[]});
  ...
  return(
    <Card style={{marginBottom:14,border:`1px solid ${C.accent}44`}}>
      ...
      <TA value={f.summary} onChange={e=>s("summary",e.target.value)} placeholder="Resumo clínico..." rows={2} style={{marginBottom:8}}/>
...
function AreasModule(){
  const [areas,setAreas]=useState(()=>DB.get("areas")||[{id:"enfermaria",name:"Enfermaria",icon:"🏥",color:C.accent,permanent:true,createdAt:new Date().toISOString()}]);
  ...
  if(selectedArea){
    ...
    return(
      <div style={{animation:"fadeIn 0.3s ease"}}>
        ...
        {showNewPatient&&<NewPatientForm onSave={p=>{savePts([...patients,{...p,id:Date.now().toString(),areaId:selectedArea,createdAt:new Date().toISOString()}]);setShowNewPatient(false);}} onCancel={()=>setShowNewPatient(false)}/>}
        {pts.length===0?<Card style={{textAlign:"center",color:C.textMuted,padding:40}}>Nenhum paciente.<br/><span style={{fontSize:12}}>Clique em "+ Paciente" para adicionar.</span></Card>
          :pts.map(p=><PatientCard key={p.id} patient={p} onClick={()=>setSelectedPatient(p)} onDelete={()=>savePts(patients.filter(x=>x.id!==p.id))}/>)}
      </div>
    );
  }
      ...
      {search?(
        ...
        <div style={{display:"grid",gap:10}}>
          {areas.map(area=>{
            ...
            return(
              <Card key={area.id} onClick={()=>setSelectedArea(area.id)} style={{cursor:"pointer",borderLeft:`3px solid ${area.color||C.accent}`}}>
...
function PatientDetail({patient,onBack,onUpdate}){
  ...
  const TABS=[{id:"caso",label:"📋 Caso"},{id:"evolucao",label:"📝 Evolução"},{id:"pendencias",label:`⏳${(form.pending||[]).length?` (${form.pending.length})`:""} Pendências`},{id:"passagem",label:"🗣️ Passagem"},{id:"ia",label:"🤖 IA"}];
...
function ChatModule(){
  ...
  const handleFile=f=>{if(!f)return;setFile(f);if(f.type.startsWith("image/")){const r=new FileReader();r.onload=e=>setPreview(e.target.result);r.readAsDataURL(f);}else setPreview(null);};
  const send=async()=>{
    ...
    try{
      ...
      const r=await callGemini({prompt:hist,systemPrompt:SYS,imageBase64:ib,imageType:it,audioBase64:ab});
      const upd=[...nm,{role:"assistant",content:r}];setMsgs(upd);localStorage.setItem("chatHistory",JSON.stringify(upd.slice(-50)));
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 128px)"}}>
      ...
      <div style={{borderTop:`1px solid ${C.border}`,paddingTop:10}}>
        {preview&&<img src={preview} alt="" style={{height:50,borderRadius:8,marginBottom:8}}/>}
...
// ── SETTINGS ────────────────────────────────────────────────
function SettingsModule({onReset}){
  ...
  return(
    <div>
      ...
      <Card style={{marginBottom:10}}>
        ...
        {workingModel&&<div style={{marginTop:8,fontSize:11,color:C.textMuted}}>Modelo ativo: <span style={{color:C.green}}>{workingModel}</span></div>}
      </Card>
      <Card style={{marginBottom:10}}>
      ...
      <Card>
        <div style={{fontWeight:700,color:C.red,marginBottom:6}}>⚠️ Limpar dados</div>
        <div style={{fontSize:12,color:C.textMuted,marginBottom:8}}>Apaga histórico de chat, pacientes e todos os dados salvos.</div>
        <Btn onClick={()=>{if(window.confirm("Limpar TODOS os dados? Esta ação não pode ser desfeita.")){localStorage.clear();onReset();}}} variant="danger" size="sm">🗑️ Limpar tudo</Btn>
      </Card>
    </div>
  );
}
  ...
  return(
    <div style={{fontFamily:"'Georgia',serif",background:C.bg,minHeight:"100vh",color:C.text,display:"flex",flexDirection:"column",maxWidth:800,margin:"0 auto"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:14,paddingBottom:82,animation:"fadeIn 0.3s ease"}}>
        ...
        {tab==="drug"&&<DrugModule/>}
        {tab==="prescriptions"&&<PrescriptionsModule/>}
        {tab==="upload"&&<UploadModule/>}
        {tab==="reports"&&<ReportsModule/>}
        {tab==="biblioteca"&&<BibliotecaModule/>}
        {tab==="cientista"&&<MedicoCientistaModule/>}
        {tab==="settings"&&<SettingsModule onReset={()=>setReady(false)}/>}
      </div>
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:800,background:C.surface,borderTop:`1px solid ${C.border}`,display:"flex",overflowX:"auto",zIndex:100}}>
        {TABS.map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{flex:"0 0 auto",minWidth:58,padding:"8px 4px 6px",background:tab===t.id?C.accentDim:"transparent",border:"none",borderTop:tab===t.id?`2px solid ${C.accent}`:"2px solid transparent",color:tab===t.id?C.accent:C.textMuted,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}><span style={{fontSize:16}}>{t.icon}</span><span style={{fontSize:8,fontWeight:700,letterSpacing:0.3}}>{t.label}</span></button>)}
      </div>
    </div>
  );
}
