...
const saveKey = (key) => {
  ...
  document.cookie = `geminiKey=${encodeURIComponent(key)};max-age=31536000;path=/;SameSite=Strict`;
};
const savePinecone = (apiKey, host) => {
  if(apiKey) { localStorage.setItem("pineconeKey", apiKey); document.cookie = `pineconeKey=${encodeURIComponent(apiKey)};max-age=31536000;path=/;SameSite=Strict`; }
  if(host) { localStorage.setItem("pineconeHost", host); document.cookie = `pineconeHost=${encodeURIComponent(host)};max-age=31536000;path=/;SameSite=Strict`; }
};
// Recupera todas as chaves de cookies se localStorage estiver vazio
(() => {
  const getCookie = (name) => { const c = document.cookie.split(";").find(c=>c.trim().startsWith(name+"=")); return c ? decodeURIComponent(c.split("=").slice(1).join("=")) : null; };
  if (!localStorage.getItem("geminiKey")) { const k=getCookie("geminiKey"); if(k){localStorage.setItem("geminiKey",k);sessionStorage.setItem("geminiKey",k);} }
  if (!localStorage.getItem("pineconeKey")) { const k=getCookie("pineconeKey"); if(k)localStorage.setItem("pineconeKey",k); }
  if (!localStorage.getItem("pineconeHost")) { const k=getCookie("pineconeHost"); if(k)localStorage.setItem("pineconeHost",k); }
})();
...
function AreaAIBar({areaId, patients, onSavePts, onAddPatient, onUpdatePatient}){
  ...
  const handleFiles=(newFiles)=>{
    ...
    setStatus("📸 "+imgs.length+" foto(s) pronta(s) — toque em ➤ para extrair");
  };

  const process=async()=>{
    if((!input.trim()&&files.length===0)||loading)return;
    setLoading(true);setStatus("🤖 Analisando...");
    ...
    for(let i=0;i<files.length;i++){
      ...
      try{
        ...
        setStatus(`🔍 Lendo imagem ${i+1}/${files.length}...`);
        const ir=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${key}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{role:"user",parts:[{inline_data:{mime_type:file.type,data:b64}},{text:"Extraia TODOS os dados médicos desta imagem: nome do paciente, leito, medicamentos (nome, dose, via, horário, dia de ATB), laboratório (nome do exame, valor), sinais vitais (PA, FC, FR, SpO2, temperatura), diagnósticos, comorbidades, prescrições. Liste tudo numericamente."}]}],generationConfig:{maxOutputTokens:1500,temperature:0.1}})});
        const id=await ir.json();
        imgContext+=`\n[Foto ${i+1}]: `+(id.candidates?.[0]?.content?.parts?.[0]?.text||"")+`\n`;
      }catch(e){console.warn("img",i,"failed:",e);}
    }

    try{
      setStatus("🧠 Processando dados...");
      const promptText=`Você gerencia pacientes de uma área hospitalar.

PACIENTES DESTA ÁREA:
${list}

COMANDO: "${input||"(use apenas os dados das imagens)"}"
${imgContext?`\nDADOS DAS IMAGENS:${imgContext}`:""}

Responda APENAS com JSON válido:
{
  "patients_to_create": [{"name":"","age":"","gender":"M|F|O","bed":"","status":"stable|attention|critical|intercurrence|discharge","summary":"","comorbidities":"","medications":[{"name":"","dose":"","route":"","schedule":"","isAtb":false,"atbDay":""}],"pending":[],"vitals":{},"labs":{},"history":"","admissionDate":"YYYY-MM-DD"}],
  "patients_to_update": [{"id":"ID","changes":{"status":"","summary":"","clinicalImpression":"","pending_add":[],"medications_add":[],"vitals":{},"labs":{},"todayEvolution":""}}],
  "patients_to_remove": [],
  "message": "confirmação em 1 frase"
}
Inclua APENAS campos mencionados (null para o resto). Para labs use chaves: hb,ht,leuco,plaq,cr,ur,na,k,glicemia,pcr,lactato,tni,bnp,inr,ast,alt.`;

      const res=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${key}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{role:"user",parts:[{text:promptText}]}],generationConfig:{maxOutputTokens:2000,temperature:0.1}})});
      const d=await res.json();if(d.error)throw new Error(d.error.message);
      const result=JSON.parse((d.candidates?.[0]?.content?.parts?.[0]?.text||"{}").replace(/```json|```/g,"").trim());

      let allPts=[...patients];

      if(result.patients_to_create?.length){
        result.patients_to_create.forEach(p=>{
          const np={...p,id:Date.now().toString()+Math.random().toString(36).slice(2),areaId,createdAt:new Date().toISOString(),medications:p.medications||[],pending:p.pending||[],labs:p.labs||{},vitals:p.vitals||{}};
          allPts.push(np);if(onAddPatient)onAddPatient(np);
        });
      }
      if(result.patients_to_update?.length){
        result.patients_to_update.forEach(upd=>{
          const i=allPts.findIndex(p=>p.id===upd.id);if(i<0)return;
          const p=allPts[i];const ch=upd.changes||{};let u={...p};
          ...
          if(ch.labs&&Object.keys(ch.labs).length){u.labs={...(u.labs||{}),...Object.fromEntries(Object.entries(ch.labs).filter(([,v])=>v))};u.labsDate=new Date().toISOString();}
          ...
          allPts[i]=u;if(onUpdatePatient)onUpdatePatient(u);
        });
      }
      if(result.patients_to_remove?.length)allPts=allPts.filter(p=>!result.patients_to_remove.includes(p.id));
  ...
  return(
    <div style={{marginBottom:12}}>
      ...
      <input ref={galleryRef} type="file" accept="image/*,application/pdf" multiple style={{display:"none"}} onChange={e=>handleFiles(e.target.files)}/>

      {/* Botões câmera + galeria */}
      <div style={{display:"flex",gap:6,marginBottom:8}}>
        <button onClick={()=>cameraRef.current?.click()} style={{flex:1,background:"rgba(99,179,237,0.08)",border:`1px solid ${C.accent}33`,borderRadius:10,color:C.accent,padding:"10px 0",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>📸 Câmera</button>
        <button onClick={()=>galleryRef.current?.click()} style={{flex:1,background:"rgba(99,179,237,0.08)",border:`1px solid ${C.accent}33`,borderRadius:10,color:C.accent,padding:"10px 0",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>🖼️ Galeria</button>
      </div>
      ...
      {previews.length>0&&(
        <div style={{marginBottom:8}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
            <span style={{fontSize:11,color:C.accent,fontWeight:700}}>📸 {previews.length} arquivo(s)</span>
            <button onClick={()=>{setFiles([]);setPreviews([]);setStatus("");}} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontSize:11,fontWeight:700}}>Limpar</button>
          </div>
          <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:3}}>
            {previews.map((p,i)=>(
              <div key={i} style={{position:"relative",flexShrink:0}}>
                {p?<img src={p} alt="" style={{height:60,width:60,borderRadius:8,objectFit:"cover",border:`2px solid ${C.accent}33`}}/>:<div style={{height:60,width:60,background:C.accentDim,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>📄</div>}
                <div style={{position:"absolute",bottom:1,left:2,background:"rgba(0,0,0,0.7)",borderRadius:3,padding:"1px 3px",fontSize:8,color:"#fff"}}>{i+1}</div>
                <button onClick={()=>{setFiles(f=>f.filter((_,j)=>j!==i));setPreviews(p=>p.filter((_,j)=>j!==i));}} style={{position:"absolute",top:-4,right:-4,background:C.red,border:"none",borderRadius:"50%",color:"#fff",width:17,height:17,cursor:"pointer",fontSize:9,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700}}>✕</button>
              </div>
            ))}
            <div onClick={()=>galleryRef.current?.click()} style={{height:60,width:60,background:C.surface2,border:`2px dashed ${C.border}`,borderRadius:8,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>
      ...
      <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
        <textarea ref={ref} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();process();}}} placeholder={files.length>0?"Opcional: qual paciente? qual exame? contexto adicional...":"🤖 Texto ou fotos — criar João 67a leito 12 ICC / João PA 130x80 Cr 2.1 / Maria piora"} rows={2} style={{flex:1,background:"rgba(99,179,237,0.05)",border:`2px solid ${C.accent}33`,borderRadius:10,color:C.text,padding:"9px 12px",fontSize:12,fontFamily:"inherit",resize:"none",boxSizing:"border-box"}}/>
        <button onClick={process} disabled={loading||(!input.trim()&&files.length===0)} style={{background:loading?"transparent":`linear-gradient(135deg,${C.accent},#4299e1)`,border:`2px solid ${C.accent}`,borderRadius:10,color:loading?C.accent:"#fff",padding:"9px 14px",cursor:"pointer",fontSize:20,flexShrink:0,opacity:(!input.trim()&&files.length===0)?0.4:1,height:56,alignSelf:"flex-end"}}>
      ...
      {status&&<div style={{fontSize:12,color:status.startsWith("✅")?C.green:status.startsWith("⚠️")?C.red:C.accent,marginTop:5,padding:"5px 10px",background:status.startsWith("✅")?C.greenDim:status.startsWith("⚠️")?C.redDim:C.accentDim,borderRadius:8,lineHeight:1.5}}>{status}</div>}
    </div>
  );
}
...
function SettingsModule({onReset, onLogout, userProfile, firebaseUser}){
  ...
  const [pKey,setPKey]=useState(localStorage.getItem("pineconeKey")||"");
  const [pHost,setPHost]=useState(localStorage.getItem("pineconeHost")||"");
  const [saved,setSaved]=useState(false);const [pSaved,setPSaved]=useState(false);
  const [testing,setTesting]=useState(false);const [msg,setMsg]=useState("");
  const [showExport,setShowExport]=useState(false);
  const save=()=>{saveKey(key);setSaved(true);setTimeout(()=>setSaved(false),2000);};
  const savePineconeKeys=()=>{savePinecone(pKey,pHost);setPSaved(true);setTimeout(()=>setPSaved(false),2000);};
  const test=async()=>{
    setTesting(true);setMsg("");
    try{workingModel=null;localStorage.removeItem("geminiModel");saveKey(key);await callGemini({prompt:"OK",maxTokens:10});setMsg(`✅ Funcionando! Modelo: ${workingModel}`);}
    catch(e){setMsg("❌ "+e.message);}
    setTesting(false);
  };
  // Gera QR code / link de configuração para sincronizar com celular
  const exportConfig=()=>{
    const cfg={geminiKey:localStorage.getItem("geminiKey")||key,pineconeKey:localStorage.getItem("pineconeKey")||pKey,pineconeHost:localStorage.getItem("pineconeHost")||pHost,geminiModel:localStorage.getItem("geminiModel")||""};
    const encoded=btoa(JSON.stringify(cfg));
    const url=`${window.location.origin}?cfg=${encoded}`;
    navigator.clipboard.writeText(url).then(()=>alert("✅ Link copiado! Cole no celular para sincronizar as chaves.")).catch(()=>setShowExport(url));
  };
  const importConfig=()=>{
    const url=prompt("Cole o link de configuração:");
    if(!url)return;
    try{
      const params=new URLSearchParams(new URL(url).search);
      const cfg=JSON.parse(atob(params.get("cfg")||btoa("{}")));
      if(cfg.geminiKey){saveKey(cfg.geminiKey);setKey(cfg.geminiKey);}
      if(cfg.pineconeKey){savePinecone(cfg.pineconeKey,cfg.pineconeHost);setPKey(cfg.pineconeKey);setPHost(cfg.pineconeHost);}
      if(cfg.geminiModel)localStorage.setItem("geminiModel",cfg.geminiModel);
      alert("✅ Configurações importadas com sucesso!");
    }catch(e){alert("❌ Link inválido: "+e.message);}
  };
  // Verifica se veio link de configuração na URL
  useEffect(()=>{
    const params=new URLSearchParams(window.location.search);
    const cfg=params.get("cfg");
    if(cfg){try{const c=JSON.parse(atob(cfg));if(c.geminiKey){saveKey(c.geminiKey);setKey(c.geminiKey);}if(c.pineconeKey){savePinecone(c.pineconeKey,c.pineconeHost);setPKey(c.pineconeKey);setPHost(c.pineconeHost);}if(c.geminiModel)localStorage.setItem("geminiModel",c.geminiModel);window.history.replaceState({},'',window.location.pathname);alert("✅ Configurações sincronizadas!");}catch{}}
  },[]);
  ...
  return(
    <div>
      ...
      {/* Pinecone */}
      <Card style={{marginBottom:10}}>
        <div style={{fontWeight:700,color:C.text,marginBottom:4}}>🧠 Pinecone — Biblioteca RAG</div>
        <div style={{fontSize:12,color:C.textMuted,marginBottom:10,lineHeight:1.6}}>Configure para usar a biblioteca vetorizada. As chaves ficam salvas em cookie — persistem no celular.</div>
        <Input value={pKey} onChange={e=>setPKey(e.target.value)} placeholder="API Key do Pinecone" type="password" style={{marginBottom:8}}/>
        <Input value={pHost} onChange={e=>setPHost(e.target.value)} placeholder="Host URL (https://preceptor-medico-xxx.svc...)" style={{marginBottom:10}}/>
        <div style={{display:"flex",gap:8}}>
          <Btn onClick={savePineconeKeys} variant="success" size="sm">{pSaved?"✅ Salvo!":"💾 Salvar"}</Btn>
          <Btn onClick={async()=>{try{const res=await fetch(`${pHost}/describe_index_stats`,{headers:{"Api-Key":pKey}});const d=await res.json();setMsg(`✅ Pinecone OK! ${d.totalVectorCount||0} vetores`);}catch(e){setMsg("❌ Pinecone: "+e.message);}}} variant="ghost" size="sm">🧪 Testar</Btn>
        </div>
        {pKey&&pHost&&<div style={{fontSize:11,color:C.green,marginTop:6}}>✅ Configurado — biblioteca sincroniza automaticamente</div>}
      </Card>

      {/* Sincronizar com celular */}
      <Card style={{marginBottom:10,border:`1px solid ${C.accent}33`}}>
        <div style={{fontWeight:700,color:C.text,marginBottom:4}}>📱 Sincronizar configurações com celular</div>
        <div style={{fontSize:12,color:C.textMuted,marginBottom:10,lineHeight:1.6}}>
          Gera um link com todas as suas chaves. Abra no celular para sincronizar automaticamente.
        </div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          <Btn onClick={exportConfig} variant="ghost" size="sm">📤 Gerar link de sincronização</Btn>
          <Btn onClick={importConfig} variant="ghost" size="sm">📥 Importar de link</Btn>
        </div>
        {showExport&&(
          <div style={{marginTop:10,background:C.surface2,borderRadius:8,padding:10}}>
            <div style={{fontSize:11,color:C.textMuted,marginBottom:6}}>Copie este link e abra no celular:</div>
            <div style={{fontSize:10,color:C.accent,wordBreak:"break-all",lineHeight:1.5}}>{showExport}</div>
            <Btn onClick={()=>navigator.clipboard.writeText(showExport)} variant="ghost" size="sm" style={{marginTop:6}}>📋 Copiar</Btn>
          </div>
        )}
      </Card>
...
function PlantaoTab(){
  ...
  const handleAiFiles=(newFiles)=>{
    const imgs=Array.from(newFiles).filter(f=>f.type.startsWith("image/")||f.type==="application/pdf");
    if(!imgs.length)return;
    setAiFiles(prev=>[...prev,...imgs]);
    imgs.forEach(f=>{if(f.type.startsWith("image/")){const r=new FileReader();r.onload=e=>setAiPreviews(prev=>[...prev,e.target.result]);r.readAsDataURL(f);}else setAiPreviews(prev=>[...prev,null]);});
    setAiStatus("📸 "+imgs.length+" foto(s) pronta(s) — toque ➤ para extrair");
  };

  // IA processa texto livre + imagens e age sobre os pacientes
  const processAI=async()=>{
    if((!aiInput.trim()&&aiFiles.length===0)||aiLoading)return;
    setAiLoading(true);setAiStatus("🤖 Analisando...");
    const key=getKey();const model=localStorage.getItem("geminiModel")||"gemini-2.5-flash";

    // Extrai texto das imagens
    let imgContext="";
    for(let i=0;i<aiFiles.length;i++){
      const file=aiFiles[i];if(!file.type.startsWith("image/"))continue;
      try{
        const b64=await new Promise((res,rej)=>{const r=new FileReader();r.onload=()=>res(r.result.split(",")[1]);r.onerror=rej;r.readAsDataURL(file)});
        setAiStatus(`🔍 Lendo imagem ${i+1}/${aiFiles.length}...`);
        const ir=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${key}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{role:"user",parts:[{inline_data:{mime_type:file.type,data:b64}},{text:"Extraia TODOS os dados médicos: nome do paciente, leito, medicamentos (nome, dose, via, horário, dia ATB), laboratório (exame, valor), vitais (PA, FC, FR, SpO2, temperatura), diagnósticos, comorbidades. Liste tudo."}]}],generationConfig:{maxOutputTokens:1500,temperature:0.1}})});
        const id=await ir.json();imgContext+=`\n[Foto ${i+1}]: `+(id.candidates?.[0]?.content?.parts?.[0]?.text||"")+"\n";
      }catch(e){console.warn("img",i,e);}
    }
...
COMANDO DO MÉDICO: "${aiInput||"(use os dados das imagens)"}"
${imgContext?`\nDADOS DAS IMAGENS:${imgContext}`:""}
...
- Se não souber qual paciente, use o mais parecido pelo nome`}]}],generationConfig:{maxOutputTokens:2000,temperature:0.1}})
      ...
      save(newList);
      setAiStatus("✅ "+msg);
      setAiInput("");setAiFiles([]);setAiPreviews([]);
  ...
  return(
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      {/* BARRA IA COM CÂMERA */}
      <div style={{marginBottom:12}}>
        <input ref={aiCameraRef} type="file" accept="image/*" capture="environment" multiple style={{display:"none"}} onChange={e=>handleAiFiles(e.target.files)}/>
        <input ref={aiGalleryRef} type="file" accept="image/*,application/pdf" multiple style={{display:"none"}} onChange={e=>handleAiFiles(e.target.files)}/>

        <div style={{display:"flex",gap:6,marginBottom:8}}>
          <button onClick={()=>aiCameraRef.current?.click()} style={{flex:1,background:"rgba(246,173,85,0.1)",border:`1px solid ${C.gold}33`,borderRadius:10,color:C.gold,padding:"10px 0",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>📸 Câmera</button>
          <button onClick={()=>aiGalleryRef.current?.click()} style={{flex:1,background:"rgba(246,173,85,0.1)",border:`1px solid ${C.gold}33`,borderRadius:10,color:C.gold,padding:"10px 0",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>🖼️ Galeria</button>
        </div>

        {aiPreviews.length>0&&(
          <div style={{marginBottom:8}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
              <span style={{fontSize:11,color:C.gold,fontWeight:700}}>📸 {aiPreviews.length} arquivo(s)</span>
              <button onClick={()=>{setAiFiles([]);setAiPreviews([]);}} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontSize:11,fontWeight:700}}>Limpar</button>
            </div>
            <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:3}}>
              {aiPreviews.map((p,i)=>(
                <div key={i} style={{position:"relative",flexShrink:0}}>
                  {p?<img src={p} alt="" style={{height:60,width:60,borderRadius:8,objectFit:"cover",border:`2px solid ${C.gold}44`}}/>:<div style={{height:60,width:60,background:C.goldDim,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>📄</div>}
                  <button onClick={()=>{setAiFiles(f=>f.filter((_,j)=>j!==i));setAiPreviews(p=>p.filter((_,j)=>j!==i));}} style={{position:"absolute",top:-4,right:-4,background:C.red,border:"none",borderRadius:"50%",color:"#fff",width:17,height:17,cursor:"pointer",fontSize:9,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700}}>✕</button>
                </div>
              ))}
              <div onClick={()=>aiGalleryRef.current?.click()} style={{height:60,width:60,background:C.surface2,border:`2px dashed ${C.border}`,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>
                <span style={{fontSize:16,color:C.textMuted}}>+</span>
              </div>
            </div>
          </div>
        ...
        <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
          <textarea ref={aiRef} value={aiInput} onChange={e=>setAiInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();processAI();}}} placeholder={aiFiles.length>0?"Opcional: contexto das fotos (qual paciente?)...":"Texto livre: criar João 67a leito 12 ICC / João PA 130x80 / Pedro piora crítico / dar alta Maria"} rows={2} style={{flex:1,background:`rgba(246,173,85,0.06)`,border:`2px solid ${C.gold}44`,borderRadius:10,color:C.text,padding:"9px 12px",fontSize:13,fontFamily:"inherit",resize:"none",boxSizing:"border-box"}}/>
          <button onClick={processAI} disabled={aiLoading||(!aiInput.trim()&&aiFiles.length===0)} style={{background:aiLoading?"transparent":`linear-gradient(135deg,${C.gold},#e67e22)`,border:`2px solid ${C.gold}`,borderRadius:10,color:aiLoading?C.gold:"#fff",padding:"9px 14px",cursor:"pointer",fontSize:20,flexShrink:0,opacity:(!aiInput.trim()&&aiFiles.length===0)?0.4:1,height:56,alignSelf:"flex-end"}}>
            {aiLoading?<div style={{width:18,height:18,border:`2px solid ${C.gold}33`,borderTop:`2px solid ${C.gold}`,borderRadius:"50%",animation:"spin 0.7s linear infinite"}}/>:"➤"}
          </button>
        </div>
        {aiStatus&&<div style={{fontSize:12,color:aiStatus.startsWith("✅")?C.green:aiStatus.startsWith("⚠️")?C.red:C.gold,marginTop:5,padding:"5px 10px",background:aiStatus.startsWith("✅")?C.greenDim:aiStatus.startsWith("⚠️")?C.redDim:C.goldDim,borderRadius:8,lineHeight:1.5}}>{aiStatus}</div>}
      </div>
