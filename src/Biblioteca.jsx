...
export default function BibliotecaModule() {
  ...
  const uploadRAG = async () => {
    ...
    try {
      ...
      // 2. Estratégia baseada no tamanho do arquivo
      // Arquivos grandes (>10MB): processa em blocos de páginas estimadas
      // Arquivos pequenos (<10MB): processa tudo de uma vez
      const BLOCK_SIZE_MB = 8; // processa ~8MB por vez
      const totalBlocks = Math.max(1, Math.ceil(fileSizeMB / BLOCK_SIZE_MB));

      let allChunks = [];

      if (fileSizeMB <= 10) {
        // ── Arquivo pequeno: extrai tudo de uma vez ──
        setUploadProgress({ current: 5, total: 100, phase: "📝 Extraindo texto (arquivo pequeno)..." });
        const text = await callGemini({
          prompt: `Extraia TODO o texto deste documento de forma fiel e completa. Preserve títulos, subtítulos, tabelas e estrutura. Não resuma — transcreva o conteúdo completo.`,
          pdfBase64: base64,
          maxTokens: 8000,
        });
        allChunks = splitIntoChunks(text, 400, 50);
      } else {
        // ── Arquivo grande: processa em blocos estimados ──
        setUploadProgress({ current: 0, total: totalBlocks, phase: `📚 Arquivo grande (${fileSizeMB.toFixed(0)}MB) — processando em ${totalBlocks} blocos...` });

        for (let block = 0; block < totalBlocks; block++) {
          const startPct = Math.round((block / totalBlocks) * 100);
          const endPct = Math.round(((block + 1) / totalBlocks) * 100);
          setUploadProgress({
            current: block,
            total: totalBlocks,
            phase: `📖 Bloco ${block + 1}/${totalBlocks} — páginas ~${startPct * 12}-${endPct * 12}...`,
          });

          try {
            const blockText = await callGemini({
              prompt: `Este é um PDF grande. Extraia o texto da PARTE ${block + 1} de ${totalBlocks} do documento (aproximadamente páginas ${startPct * 12 + 1} a ${endPct * 12}).
              
Instruções:
- Extraia o máximo de texto possível desta seção
- Preserve títulos, subtítulos, tabelas, listas
- Se encontrar fórmulas ou posologias, transcreva exatamente
- Não resuma — transcreva o conteúdo

Foco desta parte: ${block === 0 ? "início do documento, prefácio, primeiros capítulos" : block === totalBlocks - 1 ? "capítulos finais, índice, referências" : `capítulos do meio (parte ${block + 1})`}`,
              pdfBase64: base64,
              maxTokens: 6000,
            });

            const blockChunks = splitIntoChunks(blockText, 400, 50);
            // Adiciona metadado de bloco em cada chunk
            const enrichedChunks = blockChunks.map(c => ({
              text: c,
              block: block + 1,
              estimatedPage: Math.round(startPct * 12 + 1),
            }));
            allChunks.push(...enrichedChunks);

          } catch (blockErr) {
            console.warn(`Bloco ${block + 1} falhou:`, blockErr.message);
            // Continua para o próximo bloco em vez de abortar
            setError(`⚠️ Bloco ${block + 1} falhou (continuando): ${blockErr.message}`);
