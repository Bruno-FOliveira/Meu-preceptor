...
async function getEmbedding(text) {
  const key = getKey();
  // Modelos de embedding disponíveis em 2026 (text-embedding-004 foi desativado em Jan/2026)
  ...
  for (const model of models) {
    try {
      ...
      const data = await res.json();
      if (data.error) { console.log(`${model} failed:`, data.error.message); continue; }
      if (data.embedding?.values?.length > 0) return data.embedding.values;
    } catch(e) { console.log(`${model} error:`, e.message); continue; }
  }
  throw new Error("Embedding indisponível. Use modo PDF Direto para este livro.");
}
...
function ProgressBar({ value, max, label }) {
  ...
  const isBlocks = label && label.includes("bloco");
  return (
    <div style={{ marginTop: 12, background: C.surface2, border: `1px solid ${C.accent}33`, borderRadius: 12, padding: 14 }}>
      <div style={{ fontSize: 13, color: C.accent, fontWeight: 600, marginBottom: 8, lineHeight: 1.4 }}>{label}</div>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: C.textMuted, marginBottom: 6 }}>
        <span>{isBlocks ? `Bloco ${value} de ${max}` : `${value} de ${max} chunks`}</span>
        <span style={{ fontWeight: 700, color: C.accent }}>{pct}%</span>
      </div>
      <div style={{ background: C.bg, borderRadius: 8, height: 10, overflow: "hidden" }}>
        <div style={{ background: `linear-gradient(90deg,${C.accent},#4299e1)`, height: "100%", width: `${pct}%`, transition: "width 0.5s ease", borderRadius: 8, boxShadow: `0 0 8px ${C.accent}66` }} />
      </div>
      <div style={{ fontSize: 11, color: C.textDim, marginTop: 6 }}>
        ⚠️ Não feche o app — processamento em andamento
      </div>
    </div>
  );
}
...
export default function BibliotecaModule() {
  ...
  const [uploadFile, setUploadFile] = useState(null);       // single file (legacy)
  const [uploadFiles, setUploadFiles] = useState([]);        // multiple files queue
  ...
  const [queueProgress, setQueueProgress] = useState({ current: 0, total: 0, currentFile: "" });
  const [multiMode, setMultiMode] = useState(false); // toggle single vs multi
  ...
  // ── UPLOAD RAG (Vetorização) com processamento em blocos ───
  const uploadRAG = async () => {
    ...
    const titleKey = uploadMeta.title.replace(/[^a-zA-Z0-9]/g, "_").slice(0, 40);

    try {
      // 1. Lê o PDF como base64
      setUploadProgress({ current: 0, total: 100, phase: "📖 Lendo arquivo PDF..." });
      const base64 = await new Promise((res, rej) => {
        const reader = new FileReader();
        reader.onload = () => res(reader.result.split(",")[1]);
        reader.onerror = rej;
      ...
      // ── ESTRATÉGIA CORRETA para PDFs grandes ──
      // O PDF INTEIRO é enviado UMA SÓ VEZ ao Gemini com instrução de extrair TUDO
      // Depois dividimos o texto extraído em chunks no browser (sem custo de API)
      // Isso evita o erro de "modelo não disponível" por excesso de chamadas com PDF grande

      let allChunks = [];

      // Para PDFs muito grandes (>15MB), usamos múltiplas extrações focadas
      // mas com TEXTO APENAS (sem reenviar o PDF) na segunda etapa
      const LARGE_FILE_THRESHOLD = 15; // MB

      if (fileSizeMB <= LARGE_FILE_THRESHOLD) {
        // ── Arquivo pequeno/médio: extrai tudo de uma vez ──
        setUploadProgress({ current: 0, total: 3, phase: `📝 Extraindo texto do PDF (${fileSizeMB.toFixed(1)}MB)...` });
        
        const text = await callGemini({
          prompt: `Extraia TODO o texto deste documento médico de forma completa e fiel.
          
IMPORTANTE:
- Transcreva o conteúdo completo, não resuma
- Preserve títulos, subtítulos, tabelas, posologias
- Inclua todos os capítulos do início ao fim
- Mantenha números, doses, valores laboratoriais exatos`,
          pdfBase64: base64,
          maxTokens: 10000,
        });
        
        setUploadProgress({ current: 1, total: 3, phase: "✂️ Dividindo em chunks..." });
        allChunks = splitIntoChunks(text, 400, 50).map(t => ({ text: t, block: 1, estimatedPage: 1 }));
        setUploadProgress({ current: 2, total: 3, phase: `✅ ${allChunks.length} chunks extraídos` });

      } else {
        // ── Arquivo GRANDE (>15MB): extrai em 2 passagens focadas ──
        // Passagem 1: primeira metade do documento
        // Passagem 2: segunda metade do documento
        // O PDF é enviado apenas 2 vezes no total

        const totalPasses = 2;
        setUploadProgress({ current: 0, total: totalPasses + 1, phase: `📚 Arquivo grande (${fileSizeMB.toFixed(0)}MB) — extração em 2 passagens...` });

        const passes = [
          {
            label: "📖 Passagem 1/2 — primeiros capítulos...",
            prompt: `Este é um documento médico grande (${fileSizeMB.toFixed(0)}MB). 
Extraia o texto da PRIMEIRA METADE do documento.

Foque em:
- Primeiros capítulos e seções
- Prefácio, introdução
- Capítulos de 1 até aproximadamente o meio do livro
- Tabelas, posologias, critérios diagnósticos desta parte

Transcreva fielmente, sem resumir. Mantenha doses e valores exatos.`,
          },
          {
            label: "📖 Passagem 2/2 — capítulos finais...",
            prompt: `Este é um documento médico grande (${fileSizeMB.toFixed(0)}MB).
Extraia o texto da SEGUNDA METADE do documento.

Foque em:
- Capítulos da segunda metade até o fim
- Emergências específicas, UTI, protocolos
- Apêndices, índice remissivo, referências
- Tabelas e posologias desta parte
...
Transcreva fielmente, sem resumir. Mantenha doses e valores exatos.`,
        ...
        for (let p = 0; p < passes.length; p++) {
          setUploadProgress({ current: p, total: totalPasses + 1, phase: passes[p].label });
          
          try {
            const passText = await callGemini({
              prompt: passes[p].prompt,
              pdfBase64: base64,
              maxTokens: 10000,
            });

            const passChunks = splitIntoChunks(passText, 400, 50).map(t => ({
              text: t,
              block: p + 1,
              estimatedPage: p === 0 ? 1 : Math.round(fileSizeMB * 15), // estimativa
            }));
            allChunks.push(...passChunks);
            
          } catch (passErr) {
            // Se falhar uma passagem, tenta com prompt mais curto
            console.warn(`Passagem ${p + 1} falhou, tentando com prompt simplificado:`, passErr.message);
            setError(`⚠️ Passagem ${p + 1} tentando novamente...`);
            await new Promise(r => setTimeout(r, 3000));
            
            try {
              const retryText = await callGemini({
                prompt: `Extraia o texto desta ${p === 0 ? "primeira" : "segunda"} parte do documento médico. Transcreva fielmente.`,
                pdfBase64: base64,
                maxTokens: 8000,
              });
              const retryChunks = splitIntoChunks(retryText, 400, 50).map(t => ({
                text: t, block: p + 1, estimatedPage: 1,
              }));
              allChunks.push(...retryChunks);
              setError(""); // limpa erro se retry funcionou
            } catch (retryErr) {
              console.error(`Passagem ${p + 1} falhou definitivamente:`, retryErr.message);
              setError(`⚠️ Passagem ${p + 1} não processada (continuando com o restante)`);
            }
          }

          // Pausa de 3s entre passagens para não sobrecarregar
          if (p < passes.length - 1) {
            await new Promise(r => setTimeout(r, 3000));
          }
        }

        setUploadProgress({ current: totalPasses, total: totalPasses + 1, phase: `✅ ${allChunks.length} chunks extraídos` });
      }

            if (allChunks.length === 0) {
        throw new Error("Nenhum texto extraído. Tente comprimir o PDF primeiro em ilovepdf.com");
      }

      // 3. Gera embeddings e indexa no Pinecone
      const total = allChunks.length;
      setUploadProgress({ current: 0, total, phase: `🧠 Gerando embeddings para ${total} chunks...` });

      const embeddingBatch = 3; // 3 embeddings por vez (mais conservador)
      let processed = 0;
      let failed = 0;

      for (let i = 0; i < allChunks.length; i += embeddingBatch) {
        const batch = allChunks.slice(i, i + embeddingBatch);

        const vectors = [];
        for (let j = 0; j < batch.length; j++) {
          const chunk = batch[j];
          const chunkText = typeof chunk === "string" ? chunk : chunk.text;
          const chunkBlock = typeof chunk === "object" ? chunk.block : 1;
          const chunkPage = typeof chunk === "object" ? chunk.estimatedPage : Math.floor((i + j) / 3) + 1;

          try {
            const embedding = await getEmbedding(chunkText);
            vectors.push({
              id: `${titleKey}_${bookId}_${i + j}`,
              values: embedding,
              metadata: {
                title: uploadMeta.title,
                type: uploadMeta.type,
                subject: uploadMeta.subject,
                text: chunkText.slice(0, 1000), // Pinecone limita metadata
                page: chunkPage,
                block: chunkBlock,
                chunkIndex: i + j,
              },
            ...
            failed++;
            console.warn(`Embedding ${i + j} falhou:`, embErr.message);
          }

          // Pausa pequena entre embeddings
        ...
        if (vectors.length > 0) {
          try {
            ...
            console.warn("Pinecone upsert falhou:", upsertErr.message);
            // Tenta de novo após 2s
            await new Promise(r => setTimeout(r, 2000));
            try { await pineconeUpsert(vectors); } catch { failed += vectors.length; }
          }
        }
        ...
        setUploadProgress({
          current: processed,
          total,
          phase: `⚡ Indexando ${processed}/${total} chunks${failed > 0 ? ` (${failed} falhas)` : ""}...`,
        });

        await new Promise(r => setTimeout(r, 400));
      }

      const successChunks = total - failed;
      ...
      setUploadMeta({ title: "", type: "livro", subject: "" });
      setTab("biblioteca");
      alert(`✅ "${book.title}" indexado! ${successChunks}/${total} chunks${failed>0?` (${failed} falhas)`:""}. Disponível para consulta.`);
    } catch (e) {
  ...
  // ── UPLOAD MÚLTIPLO — processa fila de PDFs ────────────────
  const uploadMultipleRAG = async (files) => {
    if (!files || files.length === 0) return;
    const key = localStorage.getItem("pineconeKey");
    const host = localStorage.getItem("pineconeHost");
    if (!key || !host) { setError("Configure as chaves do Pinecone na aba ⚙️ Config RAG"); return; }
    ...
    for (let fi = 0; fi < files.length; fi++) {
      ...
      setQueueProgress({ current: fi + 1, total: totalFiles, currentFile: file.name });
      setUploadProgress({ current: 0, total: 3, phase: `📄 [${fi+1}/${totalFiles}] ${file.name}` });
      ...
      try {
        ...
        // Extrai texto — arquivo pequeno (~5MB por parte = ~100 págs)
        setUploadProgress({ current: 0, total: 3, phase: `📝 [${fi+1}/${totalFiles}] Extraindo texto de ${file.name}...` });
        
        let extractedText = "";
        try {
          extractedText = await callGemini({
            prompt: `Extraia TODO o texto deste documento médico de forma completa e fiel.
Preserve títulos, subtítulos, tabelas, posologias, doses e valores laboratoriais.
Não resuma — transcreva o conteúdo completo do início ao fim.`,
            pdfBase64: base64,
            maxTokens: 10000,
          });
        } catch (extractErr) {
          ...
          await new Promise(r => setTimeout(r, 2000));
          extractedText = await callGemini({
            prompt: "Extraia o texto completo deste documento médico.",
            pdfBase64: base64,
            maxTokens: 8000,
          });
        }
        ...
        if (!extractedText || extractedText.length < 100) {
          setError(`⚠️ ${file.name}: texto muito curto, pulando`);
          continue;
        }

        // Divide em chunks
        setUploadProgress({ current: 1, total: 3, phase: `✂️ [${fi+1}/${totalFiles}] Dividindo em chunks...` });
        const chunks = splitIntoChunks(extractedText, 400, 50);
        const bookId = Date.now().toString() + fi;
        const titleKey = fileTitle.replace(/[^a-zA-Z0-9]/g, "_").slice(0, 30);

        // Gera embeddings e indexa
        setUploadProgress({ current: 2, total: chunks.length + 2, phase: `🧠 [${fi+1}/${totalFiles}] Indexando ${chunks.length} chunks...` });

        let processed = 0; let failed = 0;
        const embBatch = 3;

        for (let i = 0; i < chunks.length; i += embBatch) {
          const batch = chunks.slice(i, i + embBatch);
          const vectors = [];

          for (let j = 0; j < batch.length; j++) {
            try {
              const emb = await getEmbedding(batch[j]);
              vectors.push({
                id: `${titleKey}_${bookId}_${i+j}`,
                values: emb,
                metadata: {
                  title: uploadMeta.title || fileTitle,
                  type: uploadMeta.type || "livro",
                  subject: uploadMeta.subject || "",
          ...
          if (vectors.length > 0) {
            try { await pineconeUpsert(vectors); }
            catch { await new Promise(r => setTimeout(r, 2000)); try { await pineconeUpsert(vectors); } catch { failed += vectors.length; } }
          }
          ...
          setUploadProgress({
            ...
            phase: `⚡ [${fi+1}/${totalFiles}] ${file.name}: ${processed}/${chunks.length} chunks`,
          });
        ...
        newBooks.push({
          id: bookId,
          title: uploadMeta.title ? `${uploadMeta.title} — Parte ${fi+1}` : fileTitle,
          type: uploadMeta.type || "livro",
          subject: uploadMeta.subject || "",
          mode: "rag", indexed: true,
          chunks: chunks.length - failed,
          fileName: file.name,
          fileSizeMB,
          addedAt: new Date().toISOString(),
        });
        ...
        setError(`⚠️ Erro em ${file.name}: ${fileErr.message} — continuando...`);
        await new Promise(r => setTimeout(r, 3000));
      }

      // Pausa entre arquivos
      if (fi < files.length - 1) await new Promise(r => setTimeout(r, 2000));
    }
    ...
    if (newBooks.length > 0) {
      ...
      setUploadFiles([]);
      setMultiMode(false);
      setQueueProgress({ current: 0, total: 0, currentFile: "" });
      setTab("biblioteca");
      alert(`✅ ${newBooks.length}/${totalFiles} arquivos indexados com sucesso!`);
    }
    setUploading(false);
  };
    ...
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      ...
      {tab === "upload" && (
        <div>
          ...
          {/* Toggle: arquivo único vs múltiplos */}
          {uploadMode === "rag" && (
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
              <button onClick={() => setMultiMode(false)} style={{ flex: 1, padding: "9px", borderRadius: 10, border: `1px solid ${!multiMode ? C.accent : C.border}`, background: !multiMode ? C.accentDim : "transparent", color: !multiMode ? C.accent : C.textMuted, cursor: "pointer", fontFamily: "inherit", fontWeight: 700, fontSize: 13 }}>
                📄 1 arquivo
              </button>
              <button onClick={() => setMultiMode(true)} style={{ flex: 1, padding: "9px", borderRadius: 10, border: `1px solid ${multiMode ? C.green : C.border}`, background: multiMode ? C.greenDim : "transparent", color: multiMode ? C.green : C.textMuted, cursor: "pointer", fontFamily: "inherit", fontWeight: 700, fontSize: 13 }}>
                📚 Múltiplos arquivos
              </button>
            </div>
          )}

          {/* Drop zone — múltiplos arquivos */}
          {uploadMode === "rag" && multiMode ? (
            <div>
              <div
                onClick={() => multiFileRef.current?.click()}
                onDragOver={e => e.preventDefault()}
                onDrop={e => { e.preventDefault(); const files = Array.from(e.dataTransfer.files).filter(f => f.type === "application/pdf" || f.name.endsWith(".pdf")); if (files.length) setUploadFiles(prev => { const names = prev.map(f => f.name); return [...prev, ...files.filter(f => !names.includes(f.name))]; }); }}
                style={{ border: `2px dashed ${uploadFiles.length > 0 ? C.green : C.border}`, borderRadius: 14, padding: 24, textAlign: "center", cursor: "pointer", marginBottom: 10, background: uploadFiles.length > 0 ? C.greenDim : "transparent", transition: "all 0.2s" }}
              >
                <input ref={multiFileRef} type="file" accept=".pdf,application/pdf" multiple style={{ display: "none" }} onChange={e => { const files = Array.from(e.target.files); setUploadFiles(prev => { const names = prev.map(f => f.name); return [...prev, ...files.filter(f => !names.includes(f.name))]; }); }} />
                <div style={{ fontSize: 32, marginBottom: 8 }}>📂</div>
                <div style={{ color: C.textMuted, fontSize: 14, fontWeight: 600 }}>Arraste os PDFs aqui ou clique para selecionar</div>
                <div style={{ color: C.textDim, fontSize: 12, marginTop: 4 }}>Selecione múltiplos arquivos de uma vez — cada um será processado individualmente</div>
              </div>
              ...
              {uploadFiles.length > 0 && (
                <div style={{ marginBottom: 12 }}>
                  <div style={{ fontSize: 12, color: C.green, fontWeight: 700, marginBottom: 8 }}>
                    ...
                    <button onClick={() => setUploadFiles([])} style={{ marginLeft: 10, background: "none", border: "none", color: C.red, cursor: "pointer", fontSize: 11 }}>Limpar tudo</button>
                  </div>
                  <div style={{ maxHeight: 200, overflowY: "auto", display: "flex", flexDirection: "column", gap: 4 }}>
                    {uploadFiles.map((f, i) => (
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px", background: C.surface2, borderRadius: 8, border: `1px solid ${C.border}` }}>
                        <span style={{ fontSize: 14 }}>📄</span>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 12, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{f.name}</div>
                          <div style={{ fontSize: 10, color: C.textMuted }}>{(f.size / 1024 / 1024).toFixed(1)} MB</div>
                        </div>
                        <button onClick={() => setUploadFiles(prev => prev.filter((_, j) => j !== i))} style={{ background: "none", border: "none", color: C.red, cursor: "pointer", fontSize: 14, flexShrink: 0 }}>✕</button>
                      </div>
                    ))}
                  </div>
              ...
              {/* Progresso da fila */}
              {uploading && queueProgress.total > 0 && (
                <div style={{ marginBottom: 10, padding: 12, background: C.accentDim, border: `1px solid ${C.accent}33`, borderRadius: 12 }}>
                  <div style={{ fontSize: 13, color: C.accent, fontWeight: 700, marginBottom: 4 }}>
                    📦 Fila: {queueProgress.current}/{queueProgress.total} arquivos
                  </div>
                  <div style={{ fontSize: 12, color: C.textMuted }}>{queueProgress.currentFile}</div>
                  <div style={{ background: C.bg, borderRadius: 6, height: 6, marginTop: 8, overflow: "hidden" }}>
                    <div style={{ background: `linear-gradient(90deg,${C.green},${C.accent})`, height: "100%", width: `${Math.round((queueProgress.current / queueProgress.total) * 100)}%`, transition: "width 0.5s ease", borderRadius: 6 }} />
                  </div>
                </div>
              )}
            </div>
          ) : (
            /* Drop zone — arquivo único */
            <div
          ...
          {error && <div style={{ color: C.red, fontSize: 12, marginBottom: 10, padding: "8px 12px", background: C.redDim, borderRadius: 8 }}>{error}</div>}
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
            {uploadMode === "rag" && multiMode ? (
              <Btn
                onClick={() => uploadMultipleRAG(uploadFiles)}
                disabled={uploading || uploadFiles.length === 0 || !localStorage.getItem("pineconeKey")}
                full size="lg"
              >
                {uploading
                  ? <><Sp /> Processando fila ({queueProgress.current}/{queueProgress.total})...</>
                  : `🧠 Vetorizar ${uploadFiles.length} arquivo${uploadFiles.length !== 1 ? "s" : ""}`
                }
              </Btn>
            ) : (
              <Btn
          ...
          {uploadMode === "rag" && (
            <div style={{ marginTop: 16, padding: 14, background: C.surface2, borderRadius: 12, fontSize: 12, color: C.textMuted, lineHeight: 1.7 }}>
              ...
              1. O Gemini extrai o texto do PDF<br />
              2. O texto é dividido em chunks de ~400 palavras<br />
              3. Cada chunk recebe um embedding vetorial<br />
              4. Os vetores são salvos no Pinecone com metadados<br />
              5. Consultas buscam os chunks mais relevantes em &lt;2s<br /><br />
              <strong style={{ color: C.green }}>✅ Múltiplos arquivos:</strong> arraste todos os 24 PDFs de uma vez — cada um é processado individualmente na fila!<br /><br />
              <strong style={{ color: C.yellow }}>⏱️ Tempo por arquivo de ~100 págs (~2MB):</strong> ~2-4 minutos
            </div>
          )}
        </div>
      )}
          ...
          <Card style={{ marginBottom: 14, border: `1px solid ${C.accent}33` }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
              <Btn onClick={() => {
                ...
                // Salva em cookie para persistir no celular
                document.cookie = `pineconeKey=${encodeURIComponent(pineconeKey)};max-age=31536000;path=/;SameSite=Strict`;
                document.cookie = `pineconeHost=${encodeURIComponent(pineconeHost)};max-age=31536000;path=/;SameSite=Strict`;
                alert("✅ Configurações salvas! Use Configurações → Sincronizar para enviar ao celular.");
