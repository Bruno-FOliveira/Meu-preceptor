...
function VisitCard({ patient, onUpdate, onEdit, onBack, onNext, onPrev, totalPatients, currentIndex }) {
  ...
  // Se chat aberto, mostra só o chat
  if (showChat) {
    return (
      <div style={{ height: "calc(100vh - 160px)", display: "flex", flexDirection: "column" }}>
        <PatientChat patient={patient} onUpdate={onUpdate} onClose={() => setShowChat(false)} />
      </div>
    );
  }
  ...
  const generateReport = async (type) => {
    ...
    const PROMPTS = {
      exam: "Gere uma solicitação de exame/laudo formal para:",
      inter: "Gere nota de evolução de intercorrência em SOAP para:",
      transfer: "Gere nota de transferência médica completa para:",
      discharge: "Gere resumo de alta hospitalar para:",
      referral: "Gere solicitação de parecer especializado para:",
    };
    const ctx = `${patient.name}, ${patient.age}a, Leito ${patient.bed || "—"}
Resumo: ${patient.summary || "—"}
Hospital: D${diasInternamento(patient.admissionDate)||"?"}
...
Impressão: ${patient.clinicalImpression||"—"}`;
    try {
      ...
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${key}`, {
        ...
        body: JSON.stringify({ contents:[{role:"user",parts:[{text:`${PROMPTS[type]}

${ctx}

Documento completo, formal, português brasileiro. Data: ${new Date().toLocaleDateString("pt-BR")}.`}]}], generationConfig:{maxOutputTokens:1500,temperature:0.2} })
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error.message);
      setReportResult(data.candidates?.[0]?.content?.parts?.[0]?.text || "");
  ...
  return (
    <div style={{ animation: "fadeIn 0.2s ease" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 14, marginBottom: 8, flexWrap: "wrap" }}>
        <Btn onClick={() => setShowChat(true)} variant="ghost" style={{ flex: 1 }}>💬 Chat</Btn>
        <Btn onClick={() => setShowReports(!showReports)} variant="ghost" style={{ flex: 1 }}>📄 Relatórios</Btn>
      </div>

      {/* Relatórios rápidos */}
      {showReports && (
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 12, marginBottom: 10 }}>
          <div style={{ fontSize: 11, color: C.textMuted, fontWeight: 700, marginBottom: 8, letterSpacing: 1 }}>📄 RELATÓRIO RÁPIDO</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 8 }}>
            {[
              {id:"exam",icon:"🔬",label:"Solicitar Exame"},
              {id:"inter",icon:"⚡",label:"Intercorrência"},
              {id:"transfer",icon:"🚑",label:"Transferência"},
              {id:"discharge",icon:"🏠",label:"Resumo Alta"},
              {id:"referral",icon:"📨",label:"Solicitar Parecer"},
            ].map(r => (
              <button key={r.id} onClick={() => generateReport(r.id)} disabled={reportLoading}
                style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 8, color: C.text, padding: "8px 6px", cursor: "pointer", fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 6, fontFamily: "inherit" }}>
                <span>{r.icon}</span>{r.label}
              </button>
            ))}
          </div>
          {reportLoading && <div style={{ color: C.accent, fontSize: 12, padding: "8px 0" }}>⏳ Gerando...</div>}
          {reportResult && (
            <div>
              <div style={{ background: C.surface2, borderRadius: 8, padding: 10, fontSize: 12, color: C.text, lineHeight: 1.7, whiteSpace: "pre-wrap", maxHeight: 200, overflowY: "auto", marginBottom: 8 }}>{reportResult}</div>
              <div style={{ display: "flex", gap: 6 }}>
                <Btn onClick={() => navigator.clipboard.writeText(reportResult)} variant="ghost" size="sm">📋 Copiar</Btn>
                <Btn onClick={() => setReportResult("")} variant="danger" size="sm">✕</Btn>
              </div>
            </div>
          )}
        </div>
